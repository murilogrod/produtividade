...
(function(root, factory) {
	...
	if (typeof define === 'function' && define.amd) {
		...
		var jQuery = (typeof window != 'undefined') ? window.jQuery : undefined; // isomorphic
																					// issue
		if (!jQuery) {
			try {
				jQuery = require('jquery');
				if (!jQuery.fn)
					jQuery.fn = {}; // isomorphic issue
			} catch (err) {
				if (!jQuery)
		...
		root.daterangepicker = factory(root, {}, root.moment || moment,
		...
		(
				this || {},
				function(root, daterangepicker, moment, $) { // 'this'
																// doesn't exist
																// on a server
					...
					var DateRangePicker = function(element, options, cb) {
						...
						if (typeof options.ranges === 'object') {
							for (range in options.ranges) {
								...
								if (this.dateLimit
										&& start.clone().add(this.dateLimit)
												.isAfter(maxDate))
						...
						if (this.singleDatePicker) {
							...
							this.container.find('.calendar.left').addClass(
									'single');
						...
						// swap the position of the predefined ranges if opens
						// right
						if (typeof options.ranges !== 'undefined'
							...
							ranges.remove();
							this.container.find('.calendar.left').parent()
									.prepend(html);
						}
						...
						this.container.find('.ranges').on(
								...
								$.proxy(this.clickCancel, this)).on(
								'click.daterangepicker', 'li',
								$.proxy(this.clickRange, this)).on(
								'mouseenter.daterangepicker', 'li',
								$.proxy(this.hoverRange, this)).on(
								'mouseleave.daterangepicker', 'li',
								$.proxy(this.updateFormInputs, this));
					...
					DateRangePicker.prototype = {
						...
						setEndDate : function(endDate) {
							...
							if (this.dateLimit
									&& this.startDate.clone().add(
											this.dateLimit).isBefore(
											this.endDate))
						...
						renderCalendar : function(side) {
							...
							// adjust maxDate to reflect the dateLimit setting
							// in order to
							// grey out end dates beyond the dateLimit
							if (this.endDate == null && this.dateLimit) {
								var maxLimit = this.startDate.clone().add(
										this.dateLimit).endOf('day');
						...
						renderTimePicker : function(side) {
							...
							if (side == 'left') {
								...
								selected = this.endDate ? this.endDate.clone()
										: this.previousRightTime.clone();
						...
						updateFormInputs : function() {
							...
							// ignore mouse movements while an above-calendar
							// text input has focus
							if (this.container.find(
									'input[name=daterangepicker_start]').is(
									":focus")
									|| this.container.find(
											'input[name=daterangepicker_end]')
											.is(":focus"))
						...
						hoverDate : function(e) {
							...
							// have the text inputs above calendars reflect the
						...
						clickDate : function(e) {
							...
							// * alternate between selecting a start and end
							// date for the range,
							// * if the time picker is enabled, apply the
							// hour/minute/second from the select boxes to the
							// clicked date
							// * if autoapply is enabled, and an end date was
							// chosen, apply the selection
							// * if single date picker mode, and time picker
							// isn't enabled, apply the selection immediately
							//
						...
						calculateChosenLabel : function() {
							...
							for ( var range in this.ranges) {
								if (this.timePicker) {
									if (this.startDate
											.isSame(this.ranges[range][0])
											&& this.endDate
													.isSame(this.ranges[range][1])) {
										customRange = false;
							...
							if (customRange) {
								this.chosenLabel = this.container.find(
										'.ranges li:last').addClass('active')
										.html();
								this.showCalendars();
							}
						},
						...
						monthOrYearChanged : function(e) {
							var isLeft = $(e.target).closest('.calendar')
						...
						timeChanged : function(e) {
							...
							var cal = $(e.target).closest('.calendar'), isLeft = cal
									.hasClass('left');
						...
						formInputsChanged : function(e) {
							var isRight = $(e.target).closest('.calendar')
									.hasClass('right');
							var start = moment(this.container.find(
									'input[name="daterangepicker_start"]')
									.val(), this.locale.format);
