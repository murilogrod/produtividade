...
@ApiModel(value = ConstantesPAE.VISAO_CONTRATO_API_MODEL_CONTRATO_ADMINISTRATIVO,
        description = "Objeto utilizado para representar o Contrato Adminstrativo no retorno as consultas realizadas sob a otica da consulta pelo Contrato."
)
public class ContratoAdministrativoDTO implements Serializable {
    ...
    @ApiModelProperty(name = ConstantesPAE.PROCESSO_ADMINISTRATIVO, accessMode = AccessMode.READ_ONLY, value = "Objeto que representa o Processo Administrativo de vinculação do contrato.")
    ...
    @ApiModelProperty(name = ConstantesPAE.UNIDADE_OPERACIONAL, required = true, value = "CGC que representa a unidade operacional do Contrato Adminstrativo.")
    ...
    public ContratoAdministrativoDTO(ContratoAdministrativo contrato) {
        ...
        if (Objects.nonNull(contrato.getProcessoAdministrativo())) {
            this.processoAdministrativoDTO = new ProcessoAdministrativoDTO(contrato.getProcessoAdministrativo());
        }

        if (Objects.nonNull(contrato.getApensosAdministrativos())) {
            contrato.getApensosAdministrativos()
                    .forEach(apenso -> this.apensosAdministrativosDTO.add(new ApensoAdministrativoDTO(apenso)));
        }
        if (Objects.nonNull(contrato.getDocumentosAdministrativos())) {
