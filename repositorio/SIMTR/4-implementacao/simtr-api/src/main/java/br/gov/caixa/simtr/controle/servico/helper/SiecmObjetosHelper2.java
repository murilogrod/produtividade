...
public class SiecmObjetosHelper2 {
    ...
    private DocumentoDTO montaObjetoDocumentoDTO(Documento documentoSIMTR, String binario, String classePadrao) {
        ...
        List<CampoDTO> camposDocumentoECM = montarListaCampo(documentoSIMTR);
	...
	private List<CampoDTO> montarListaCampo(Documento documentoSIMTR) {
		List<CampoDTO> camposDocumentoECM = new ArrayList<>();        
        
        for (AtributoDocumento ad : documentoSIMTR.getAtributosDocumento()) {
            CampoDTO campo = new CampoDTO();
            campo.setNome(ad.getDescricao());
            String retorno = ad.getConteudo() == null ? "" : ad.getConteudo();
             if (ad.getOpcoesSelecionadas() != null && !ad.getOpcoesSelecionadas().isEmpty()) {
            	 retorno = "[";
                 for (OpcaoSelecionada os : ad.getOpcoesSelecionadas()) {
                	 retorno.concat(os.getValorOpcao());
                	 retorno.concat("=");
                	 retorno.concat(os.getDescricaoOpcao());
                 }
                 retorno.concat("]");
             }
             campo.setValor(retorno);
