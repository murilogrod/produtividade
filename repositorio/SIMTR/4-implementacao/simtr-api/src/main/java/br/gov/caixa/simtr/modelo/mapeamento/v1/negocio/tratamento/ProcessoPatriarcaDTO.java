...
@XmlRootElement(name = ConstantesNegocioTratamento.XML_ROOT_ELEMENT_PROCESSO_PATRIARCA)
@XmlAccessorType(XmlAccessType.FIELD)
@ApiModel(
        value = ConstantesNegocioTratamento.API_MODEL_V1_PROCESSO_PATRIARCA,
        description = "Objeto utilizado para representar o macro processo no retorno as consultas realizadas para o tratamento"
)
public class ProcessoPatriarcaDTO implements Serializable {
    ...
    @ApiModelProperty(name = ConstantesNegocioTratamento.PROCESSOS_FILHO, required = false, value = "Lista de processos filho vinculados neste processo.")
    private List<ProcessoDossieDTO> processosFilhoDTO;

    public ProcessoPatriarcaDTO() {
        super();
        this.quantidadeDossies = 0;
        this.processosFilhoDTO = new ArrayList<>();
    }

    public ProcessoPatriarcaDTO(Processo processo) {
        this();
        if (processo != null) {
            ...
            if (processo.getRelacoesProcessoVinculoPai() != null) {
                ...
                relacoes.forEach(relacaoProcesso -> {
                    if (relacaoProcesso.getProcessoFilho() != null) {
                        ...
                        this.processosFilhoDTO.add(new ProcessoDossieDTO(relacaoProcesso.getProcessoFilho()));
                    }
                });
            }
