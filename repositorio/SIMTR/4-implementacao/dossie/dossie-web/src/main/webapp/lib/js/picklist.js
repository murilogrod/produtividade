function fxPickList($compile, $templateCache){
	...
	return {
		...
		restrict: 'E',
		terminal:false,
		scope:false,//do not pollute parent scope
		transclude: true,
		controller: function($scope, $element, $attrs, $parse){
			 ...
			 * Remove from right all elements in left;
			 */
			$scope.removeLeftFromRightProp=function(left, right, property){
				for(var i=0; i<left.length; i++){
					for (var j=0; j<right.length; j++){
						if (left[i][property]===right[j][property]){
							right.splice(j, 1);
							break;
						}
					}
...
var fxPickListTpl=
	...
	"					<button type=\"button\" class=\"btn btn-primary\" data-ng-click=\"rightShift();\">"+
	"						<i class=\"glyphicon glyphicon-chevron-right\"> </i>Adicionar"+
	"					</button>"+
	"				</div>"+
	"				<div style=\"text-align: center; margin-top: 12px;\">"+
	"					<button type=\"button\" class=\"btn btn-primary\" data-ng-click=\"leftShift();\">"+
	"						<i class=\"glyphicon glyphicon-chevron-left\"> </i>Remover"+
	"					</button>"+
	"				</div>"+
	"			</div>"+
	"			<div style=\"display: table-cell; width: 40%;vertical-align: middle;\">"+
	"				<select multiple id=\"id_picklist_dest\" size=\"5\" class=\"form-control\" data-ng-options=\"fake\" name=\"fake\" data-ng-model=\"picklist_dest\" data-picklist-dest>"+
	"				</select>"+
	"			</div>"+
	"		</div>"+
	"	</div>	  "
